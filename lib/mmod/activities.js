...
{
    log('________ main.dash: ' + main.dash);
    // log('________ main.panel length' + main.panel.get_children().length);
    ...
    if( this.rig.settings.get_boolean( 'panel-button-enabled' ) )
    {
        ...
        this.button = new st.Button                                // The panel button like a windows 'start' button
        (
            {
        ...
        // log('this.activities: ' + this.activities);
        // log('this.activities.actor: ' + this.activities.actor);
        this.button.connect('button-press-event', lang.bind(this, this.applicationIconClick));

        this.active = true;
    }
};

mod.prototype.applicationIconClick = function(){
    // main.Util.trySpawnCommandLine('gnome-shell-extension-prefs ' + esys.metadata.uuid);
    log('btn apps pressed');
};
